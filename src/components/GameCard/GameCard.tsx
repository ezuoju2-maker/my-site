...
export default function GameCard({ game, onClick }: { game: Game; onClick?: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="q8-game-card"
      aria-label={game.name}
    >
      <div className={`q8-game-fallback g-${game.id}`} />
      <img
        src={game.image}
        alt=""
        loading="lazy"
        onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }}
        className="q8-game-art"
      />
      <div className="q8-game-svg"><GameArt id={game.id} /></div>
      <div className="q8-game-shade" />

      {game.badge === "HOT" && <span className="q8-game-badge hot">HOT</span>}
      {game.badge === "推荐" && <span className="q8-game-badge recommend">推荐</span>}

      <span className="q8-game-name">{game.name}</span>
    </button>
  );
}
