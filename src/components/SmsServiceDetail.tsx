    ...
    () =>
      stock
        .map((s) => {
          ...
          const totalStock = getTotalStock(serviceSlug, s.countryCode, s.stock);
          return { s, c: countryMap.get(s.countryCode), priceRange, totalStock };
        })
        .filter((x): x is { s: typeof stock[0]; c: SmsCountry; priceRange: { min: number; max: number }; totalStock: number } => Boolean(x.c))
        .sort((a, b) => b.totalStock - a.totalStock),
    [stock, countryMap, serviceSlug],
  );
